...
export default function UnpaidInvoices({ allInvoices }: { allInvoices: any[] }) {
  ...
  const filteredInvoices = useMemo(() => {
    if (!searchText.trim()) return latestUnpaidInvoices;
  
    const text = searchText.toLowerCase();
  
    return latestUnpaidInvoices.filter((inv: any) => {
      const match = (field: SearchField) =>
